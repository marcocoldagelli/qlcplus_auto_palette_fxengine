#!/usr/bin/env python3
"""
qlcplus_auto_palette_fxengine.py  v1.3.1beta  Marco Coldagelli
==============================
...
Novità v1.3.1beta / What's new in v1.3.1beta:
  - FIX: lettura nome FixtureGroup robusta (tag <n>, <Name>, attributo name)
         formato label QLC-GRUPPI: "ID - Nome" / FIX: robust FixtureGroup name
         reading (tags <n>, <Name>, name attribute); QLC-GRUPPI label: "ID - Name"
...
def parse_qxw_fixture_groups(qxw_root):
    ...
    for fg in engine.findall("FixtureGroup"):
        try:
            gid = int(fg.get("ID", -1))
            # IT: QLC+ usa <n> per il nome del gruppo in alcune versioni,
            #     <n> o attributo "name" in altre. Proviamo tutte le varianti.
            # EN: QLC+ uses <n> for group name in some versions,
            #     <n> or "name" attribute in others. Try all variants.
            raw_name = None
            for tag in ("n", "Name", "name"):
                el = fg.find(tag)
                if el is not None and el.text and el.text.strip():
                    raw_name = el.text.strip()
                    break
            if raw_name is None:
                raw_name = fg.get("name") or fg.get("Name") or ""
            if raw_name:
                name = f"{gid} - {raw_name}"
            else:
                name = f"Gruppo {gid}"
...
def main():
    parser = argparse.ArgumentParser(
        description="QLC+ Auto Palette FX Engine v1.3.1beta by Marco Coldagelli",
    ...
    print(f"\n{'═'*64}")
    print(f"  QLC+ Auto Palette FX Engine v1.3.1beta by Marco Coldagelli")
